...
namespace home
{
	...
	////			auto& device = dynamic_cast<OneWireDevice>(*(Devices::GetInstance().insert(OneWireDeviceNew(address)).first));
	...
	////}
	//const OneWireDevice DJOneWire::OneWireDeviceNew(const OneWireAddress& address) const
	//{
	//	std::cout << "DJOneWire::POneWireDeviceNew()" << std::endl;
	//	std::cout << "\t" << address.StrDec() << std::endl;
	////	if (address.Array()[0] == 40)
